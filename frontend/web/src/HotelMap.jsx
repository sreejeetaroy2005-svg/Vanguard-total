...
function createFloor(floorIdx) {
  ...
  if (f === 1) {
    ...
    grid[2][0] = makeCell("stairwell", "ST");
    grid[2][11] = makeCell("stairwell", "ST");
  }
...
function bfs(grid, startR, startC, isTarget, blockedSet, mobilityImpaired) {
  ...
  while (queue.length) {
    ...
    for (const [dr, dc] of [[0, 1], [0, -1], [1, 0], [-1, 0]]) {
      ...
      if (mobilityImpaired && (grid[nr][nc].type === "stair" || grid[nr][nc].type === "stairwell")) continue;
...
function computeExitPlan(floors, roomLabel, blockedSets, isWheelchairActive) {
  ...
  const isConnector = cell => {
    ...
    return cell.type === T.STAIR || cell.type === T.ELEVATOR || cell.type === "stairwell";
  };
  const isExit = cell => cell.type === T.EXIT;
  const segments = [];
  if (roomFloor === 0) {
    const path = bfs(floors[0], roomR, roomC, isExit, blockedSets[0], isWheelchairActive);
    if (path) segments.push({ floorIdx: 0, path });
  } else {
    const path1 = bfs(floors[roomFloor], roomR, roomC, isConnector, blockedSets[roomFloor], isWheelchairActive);
    if (!path1) return null;
    segments.push({ floorIdx: roomFloor, path: path1 });
    let last = path1[path1.length - 1];
    for (let f = roomFloor - 1; f >= 0; f--) {
      if (f === 0) {
        const p = bfs(floors[0], last.r, last.c, isExit, blockedSets[0], isWheelchairActive);
        if (p) segments.push({ floorIdx: 0, path: p });
        break;
      } else {
        const p = bfs(floors[f], last.r, last.c, isConnector, blockedSets[f], isWheelchairActive);
        if (p && p.length > 0) {
...
export default function HotelMapSystem({ onClose, isMobilityImpaired }) {
  // Debug log to verify wheelchair flag arriving
  console.log('mobilityImpaired in HotelMap:', isMobilityImpaired);
  ...
  const handleCell = useCallback((r, c) => {
    ...
    if (cell.type === T.ROOM) {
      const isWheelchair = isMobilityImpaired || alerts.some(a =>
          ...
          <div className="tactical-glass p-6 rounded-3xl border border-white/5">
            ...
            {activePlan ? (
              <div className="space-y-4">
                ...
                {isMobilityImpaired && (
                  <div className="text-blue-400 text-sm font-bold mb-2 text-center">
                    ♿ Accessible Route Active
                  </div>
                )}
          ...
          <div className="flex gap-2">
            {[0, 1, 2].map(i => (
              <button
                ...
                className={`px-6 py-3 rounded-t-2xl text-[9px] font-black uppercase tracking-widest transition-all cursor-pointer ${selFloor === i
          ...
          <div className="tactical-glass p-8 rounded-b-3xl rounded-tr-3xl flex-1 flex items-center justify-center bg-zinc-950/20 border border-white/5 overflow-auto">
            <div className="grid gap-2 p-4 rounded-2xl bg-black/60 border border-white/5" style={{ gridTemplateColumns: `repeat(${COLS}, 54px)` }}>
              {curGrid.map((row, r) => row.map((cell, c) => {
                ...
                const isWheelchair = isMobilityImpaired;
                const isBlockedStair = isMobilityImpaired && (cell.type === T.STAIR || cell.type === "stairwell");
                ...
                return (
                  <div
                    ...
                    className={`w-[54px] h-[54px] rounded-xl border flex flex-col items-center justify-center transition-all duration-300 ${cell.type === T.ROOM ? 'cursor-pointer' : ''
                      } ${isBlocked
                        ...
                        : isBlockedStair
                          ...
                          : isLit && isMobilityImpaired
                            ? 'shadow-[0_0_15px_rgba(59,130,246,0.3)] border-[#3B82F6] bg-[#3B82F6]/25 scale-105'
                            : isLit
                  ...
                  >
                    {isBlocked ? (
                      ...
                      <span className="text-lg font-black animate-pulse text-red-500">✕</span>
                    ) : (
                      <>
                        ...
                        {(cell.type === T.STAIR || cell.type === "stairwell") && <span className="text-lg">🪜</span>}
